import { Signal, SignalState } from '@captainpants/sweeter-core';
import {
...
export function listenWhileNotCollected<TElement extends object, T>(
    ...
    // callback (maybe) holds a strong reference to parentNode, so we can't allow
    // a strong reference from flattenedChildrenSignal to it..
    const unlisten = signal.listenWeak(callback);
    whenGarbageCollected(lifetimeElement, unlisten);
