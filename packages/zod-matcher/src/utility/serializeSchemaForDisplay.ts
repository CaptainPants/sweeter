...
function objectForDisplay(obj: z.AnyZodObject, depth: number): string {
    ...
    for (const key of keys) {
        ...
        res.push(JSON.stringify(key) + ':' + serializeSchemaForDisplay(keySchema, depth - 1));
    }
...
const convertors = [
    ...
    create(isArrayType, (val, depth) => serializeSchemaForDisplay(val.element, deeper(depth)) + '[]'),
    create(isUnionType, (val, depth) => unionForDisplay(val, deeper(depth)))
];

export function serializeSchemaForDisplay(schema: z.ZodType, depthLimit: number = 20): string {
