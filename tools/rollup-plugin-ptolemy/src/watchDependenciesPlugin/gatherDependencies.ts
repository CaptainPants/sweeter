...
export async function gatherDependencies(
    ...
    for (const item of topLevelEntries) {
        ...
        if (item.name.startsWith('@')) {
            ...
            for (const scopedItem of await fsPromises.readdir(scopedDir, {
                withFileTypes: true,
            })) {
                const scopedItemLocation =
                    await getRealLocationForDirectory(scopedItem);

                if (scopedItemLocation === null) {
                    continue;
                }

                const scopedCandidatePackageJson = path.resolve(
                    scopedItemLocation,
                    'package.json',
                );
                if (await fileExists(scopedCandidatePackageJson)) {
                    dependencies.push({
                        name: item.name + '/' + scopedItem.name,
                        realPath: scopedItemLocation,
                    });
                }
            }
