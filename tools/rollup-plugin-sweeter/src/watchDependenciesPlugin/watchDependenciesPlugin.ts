    ...
    return {
        ...
        async buildStart() {
            ...
            for (const current of whereMatched) {
                ...
                const processor = (filename: string) => {
                    this.info(`Updating as a result of change to file ${filename}`);
                ...
                const listener: (...args: FSWatcherEventMap['all']) => void = (eventName, eventPath) => {
                    ...
                    debuglog(`File watch event ${eventName} at ${normalizedRelativePath}`);
        ...
        async closeBundle() {
            ...
            if (buildCompleteNoticePath) {
                this.info(`Bundle closed during watch, updating ${buildCompleteNoticePath}`);
                await fsPromises.writeFile(buildCompleteNoticePath, `Generated by bundle close event in watchDependenciesPlugin at ${Date.now()}`, { flush: true });
            }
        }
    };
}
